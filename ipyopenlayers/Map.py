...
class WebGLWindGradientLayer(Layer):
    _view_name = Unicode('WebGLWindGradientLayerView').tag(sync=True)
    _model_name = Unicode('WebGLWindGradientLayerModel').tag(sync=True)
    uvBuffer = Tuple().tag(sync=True)
    opacity = Float(1.0).tag(sync=True)
    updateWhileInteracting = Bool(True).tag(sync=True)
    style = Unicode('barbs').tag(sync=True)
